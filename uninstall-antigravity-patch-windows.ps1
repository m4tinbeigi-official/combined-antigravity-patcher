#!/usr/bin/env pwsh
# ==============================================================================
# uninstall-antigravity-patch-windows.ps1 - Uninstaller (Windows)
# ==============================================================================
# Restores all original files backed up by install-antigravity-patch-windows.ps1
# Usage: pwsh ./uninstall-antigravity-patch-windows.ps1
# ==============================================================================
...
if (-not $agPath) {
    $agPath = Read-Host "  Enter Antigravity installation path (or press Enter to skip)"
...
if ($backups.Count -eq 0) {
    ...
    if ($restored.Count -eq 0) {
        Write-Warn "No files restored (path not set or no matching files)."
    }
}

Write-Step "Cleaning up..."
$confirm = Read-Host "  Delete backup directory? (y/N)"
